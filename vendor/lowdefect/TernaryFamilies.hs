...
supersedes (MFam bc1 c1 l1) (MFam bc2 c2 l2) = let (q,r) = c2 `divMod` c1 in
	...
	length l1 == length l2 &&
	matchsupersedes l1 l2

matchsupersedes :: [TFam] -> [TFam] -> Bool
--Equivalent to any (and . zipWith supersedes l1) (permutations l2), but
--backtracks directly and prunes impossible child ranks before recursing.
matchsupersedes [] [] = True
matchsupersedes [] _ = False
matchsupersedes _ [] = False
matchsupersedes (f:fs) gs = any try choices
	where choices = pickmatches f gs
	      try (g,rest) = supersedes f g && matchsupersedes fs rest

pickmatches :: TFam -> [TFam] -> [(TFam,[TFam])]
pickmatches _ [] = []
pickmatches f (g:gs) =
	(if rank f == rank g then [(g,gs)] else []) ++
	map (\(h,rest)->(h,g:rest)) (pickmatches f gs)
...
rm3dup1 l = concatMap (rm3dup1sameKey . map snd) groups
	where keyed = sortBy keycmp [(supersedeskey f, f) | f <- l]
	      groups = groupBy (\f g -> fst f == fst g) keyed
	      keycmp f g = compare (fst f) (fst g)

rm3dup1sameKey :: [TFam] -> [TFam]
rm3dup1sameKey l = filter (\g->not (any (supersedesstrictcheap g) l)) l

supersedesstrictcheap :: TFam -> TFam -> Bool
supersedesstrictcheap g f = bcp f <= bcp g && supersedesstrict f g

data SupersedesKey = ASKey Integer SupersedesKey |
	MSKey Integer [SupersedesKey] deriving (Eq, Ord)

supersedeskey :: TFam -> SupersedesKey
--If f supersedes g, then supersedeskey f == supersedeskey g.  MFam constants
--may differ only by a power of 3, so only their 3-free part is retained here.
supersedeskey (AFam _ c f) = ASKey c (supersedeskey f)
supersedeskey (MFam _ c l) = MSKey (snd (splitbybase 3 c))
	(sort (map supersedeskey l))
